#!/usr/bin/env bash
# Full comparison with only FSQ-mu/FSQ-alpha64 and VQ-mu/FSQ-alpha32 splits.
set -euo pipefail

REPO_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
RUN_ROOT=${RUN_ROOT:-"${REPO_DIR}/../enhancing_ntp4jets_runs"}
CONDA_ENV=${CONDA_ENV:-/eos/home-y/yelberke/conda_condor_orbit_env}
MPLCONFIGDIR=${MPLCONFIGDIR:-"${RUN_ROOT}/matplotlib_multirun_selected_splits"}
...
MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_COMMAND[@]}" \
  ...
  --stage test \
  --group all \
  --output-dir "${RUN_ROOT}/comparisons/957028-960568-selected-splits/test/all" \
  --wandb-project orbit-tokenizer \
  --wandb-name codebook_families_957028_960568_selected_splits_test_all \
